...
nnoremap <leader>uw :cd %:p:h<cr>:pwd<cr>:silent !urxvtr -e watch %:e<cr>
nnoremap <leader>ut :cd %:p:h<cr>:pwd<cr>:silent !urxvtr -e tig<cr>
...
nnoremap <silent>[unite]b   :<C-u>Unite -buffer-name=buffer    -no-start-insert            buffer<cr>
...
nnoremap <silent>[unite]-   :<C-u>Unite -buffer-name=colorscheme    -auto-preview               colorscheme<cr>
...
nnoremap <silent><leader>cn :<C-u>UniteWithCursorWord -force-immediately -default-action=yank citation/key_raw<cr>:!urxvtr -e zotcli add-note <C-R>0<cr>
